...
add_esi_threshold <- function(df, var) {
  library(dplyr)
  library(rlang)
  ...
  notwithin <-
    df %>%
    mutate(
      is_within_discipline = if_else({{var}} %in% Threshold[[join_var]], TRUE, FALSE)
    ) %>%
    summarise(num = sum(!is_within_discipline)) %>%
    pull(num)
  ...
  df %>%
    dplyr::left_join(Threshold, by = setNames(join_var, join_var))
}
